...
- ⏸️ Anthropic / Meta AI 官方 RSS：当前站点为 JS 渲染、无公开 feed，已在 config 中以注释占位，待官方提供后启用。
- ⏸️ 跨源 LLM 聚类去重（同一事件多源合并）：留待后续。
...
- **源类型**：
  ...
  - `community`（Hacker News，Algolia API）。
...
### 3.2 `src/news_summary.py` — 打分、截断、LLM 摘要
...
- **截断**：排序后取前 `max_items`（默认 12），仅对这些调 LLM（控成本）。
- **LLM 加工** `_llm_enrich`：批量 prompt → 每条 `summary_zh`（≤40 字中文一句话）+ `category`（模型发布/研究/产品/工程/融资·商业/观点/其他）。复用 `llm_client.chat_json`（与项目分析同一套多模型可插拔配置）。
- **降级**：`llm_summarize=false` 或 LLM 不可用 → 保留空摘要，渲染层回退到原标题。
...
news:
  ...
  llm_summarize: true     # false=仅标题聚合（零 LLM 成本）
  ...
  hacker_news: true
```
新增源只改这里；新增源类型需在 `news_collect`/`news_summary` 同步权重。
...
| 后续 | Anthropic/Meta RSS、跨源 LLM 聚类去重、来源权重调优、Reddit 源 | ⏸️ 待定 |
...
## 8. 未来扩展位
- **跨源去重聚类**：同一事件被多家报道时，用 LLM/embedding 聚类合并，避免刷屏。
- **来源权重调优**：当前 arXiv 论文易占满尾部；可下调 `paper` 权重或对论文单设配额，让社区/官方新闻更靠前。
- **更多源**：Reddit（r/LocalLLaMA、r/MachineLearning）、Techmeme、各公司工程博客。
- **周资讯回顾**：参照 `trend.py`，做跨 7 天的资讯热度/主题聚合二次报告。
